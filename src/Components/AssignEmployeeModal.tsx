...
interface AvailableEmployee {
  id: string;         // The GUID from the API
  employeeId?: string; // Fallback if the API uses this name
  name?: string;
  user?: {
    fullName: string;
    image?: string;
...
const AssignEmployeeModal: React.FC<Props> = ({ isOpen, onClose, table }) => {
  ...
  const handleAssign = async () => {
    ...
    const payload = selectedEmployeeIds.map(id => ({
      employeeId: id, // Ensure this 'id' is coming from Select as a GUID string
      tableId: table.id,
    }));
    ...
    >
      ...
      >
        ...
        {availableEmployees.map((emp) => {
          // Determine which ID to use (prefer 'id', fallback to 'employeeId')
          ...
          return (
            <Select.Option
              key={actualId}
              value={actualId} // This is what gets saved in selectedEmployeeIds
              label={displayName}
            >
              <Space>
                <Avatar
